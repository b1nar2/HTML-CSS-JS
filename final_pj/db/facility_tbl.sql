...
CREATE TABLE facility_tbl (
    ...
    facility_reg_date     DATE NOT NULL DEFAULT SYSDATE,
    facility_mod_date     DATE NOT NULL DEFAULT SYSDATE, -- 디폴트값 sysdate 추가
...
-- PK 설정
ALTER TABLE facility_tbl ADD CONSTRAINT pk_facility_id PRIMARY KEY ( facility_id );


-- FK 설정
-- 회원 정보 테이블 연결
ALTER TABLE facility_tbl
ADD CONSTRAINT facility_member_fk
...
-- 사용 가능 여부 제약
ALTER TABLE facility_tbl
ADD CONSTRAINT facility_use_ch
CHECK (facility_use in ('Y', 'N'));

-- 최대/최소 인원 제약
ALTER TABLE facility_tbl
ADD CONSTRAINT facility_person_chk
