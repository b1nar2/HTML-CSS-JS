...
-- PK 설정
ALTER TABLE member_tbl ADD CONSTRAINT pk_member_id PRIMARY KEY ( member_id );
...
ADD CONSTRAINT member_gender_ch
CHECK(member_gender IN ('m', 'f'));

-- 일반유저/관리자 설정
ALTER TABLE member_tbl
ADD CONSTRAINT member_role_ch
CHECK(member_role IN ('user', 'admin'));

-- 관리자 권한 유형 설정
ALTER TABLE member_tbl
ADD CONSTRAINT member_admin_type_ch  -- chk_member_admin_type => member_admin_type_ch 변경 : 제약조건 이름 통일을 위해 변경
CHECK(
