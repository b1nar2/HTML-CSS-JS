...
@SpringBootTest
@Slf4j
public class FoodCrawlBatchJob {
	...
	/**
	 * 이미지 크롤링 배치(일괄) 작업(batch processing)
	 * 
	 * 1. 크롤링(crawling) 
	 * 2. 이미지 다운로드 (이름 암호화 : UUID) 
	 * 3. 이미지 썸네일 저장 처리 (thumb_원이미지 파일)
	 * 4. DB TABLE (FOOD_TBL) 저장 
	 */
			...
			for (int i=0; i<foodIdList.size(); i++) {
				...
				try {
					...
					if (foodImg.contains("anvil_img.jpg") == true) {
						...
						Files.copy(in, Paths.get(imagePath + saveImgFilename), StandardCopyOption.REPLACE_EXISTING);						
...
//						File outFilename = new File(path + saveImgFilename);
						...
						String slidePath = imagePath + "slide/";						
...
//						File outFilename = new File(path + saveImgFilename);
						...
						try {
							...
							// 갤러리용 썸네일 이미지
							Thumbnails.of(imagePath + saveImgFilename)
									  .size(100, 100)
